...
export const sports = pgTable(
  ...
  (table) => [unique("sports_slug_unique").on(table.slug)],
);
...
export const liveEvents = pgTable(
  ...
  (table) => [
    index("idx_live_events_created_at").using(
      "btree",
      table.createdAt.asc().nullsLast().op("timestamp_ops"),
    ),
    index("idx_live_events_match_id").using(
      "btree",
      table.matchId.asc().nullsLast().op("uuid_ops"),
    ),
    index("idx_live_events_match_sequence").using(
      "btree",
      table.matchId.asc().nullsLast().op("int4_ops"),
      table.eventSequence.asc().nullsLast().op("uuid_ops"),
    ),
    foreignKey({
